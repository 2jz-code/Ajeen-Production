...
class WebsiteTokenObtainPairView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user and user.is_website_user:
            ...
            session_identifier = get_existing_user_session(
                user, is_website=True
            ) or create_user_session(user, is_website=True)
        ...
        return Response(
            {"error": "Invalid credentials or not a website user"},
...
class WebsiteTokenRefreshView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if refresh_token:
            ...
            res = Response(
                {"message": "Guest access allowed"}, status=status.HTTP_200_OK
            )
            res.set_cookie(
                key="guest_id",
                value=guest_id,
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            user = serializer.save(
                role="customer", is_website_user=True, is_pos_user=False
            )
...
@api_view(["GET", "PUT"])
@permission_classes(
    [IsAuthenticated, IsWebsiteUser]
)  # Ensure user is authenticated and is a website user
def website_user_profile(request):
    ...
    if request.method == "GET":
        ...
        serializer = WebsiteUserSerializer(
...
def has_website_refresh_token(request):
    ...
    if refresh_token:
        try:
            ...
            if not user.is_website_user:
                return Response(
                    {"hasRefreshToken": False, "isGuest": bool(guest_id)}, status=400
                )
...
@api_view(["POST"])
@permission_classes([AllowAny])  # Should be accessible to logout
