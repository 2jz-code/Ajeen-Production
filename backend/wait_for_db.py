...
# It's better to rely on Django's settings to be fully configured
# so that it correctly parses DATABASE_URL from the environment.
# However, to avoid circular dependencies if settings try to connect
# too early, we might need a lighter check or ensure django.setup()
# doesn't trigger premature connections.

# For a direct check using psycopg2 (if DATABASE_URL is simple enough to parse or components are available)
# This is an alternative if django.setup() is problematic in a very early script.
# import psycopg2
