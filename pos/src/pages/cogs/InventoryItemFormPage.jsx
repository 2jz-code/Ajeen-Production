	...
	const fetchPageData = useCallback(async () => {
		...
		try {
			...
			if (isEditMode && itemId) {
				...
				// Logic for latest_purchase_unit_id
				const latestPurchaseUnitValue =
					itemData.latest_purchase_unit_details?.id ||
					itemData.latest_purchase_unit;
				const latestPurchaseUnitIdToSet =
					latestPurchaseUnitValue !== null &&
					latestPurchaseUnitValue !== undefined
						? latestPurchaseUnitValue.toString()
				...
				setFormData({
					...
					// Corrected: Use costing_unit_details.id or costing_unit (which is the ID)
					costing_unit_id:
						(
							itemData.costing_unit_details?.id || itemData.costing_unit
						)?.toString() || "",
					...
					// Corrected: Use latest_purchase_unit_details.id or latest_purchase_unit (which is the ID or null)
					latest_purchase_unit_id: latestPurchaseUnitIdToSet,
				});
			}
		} catch (err) {
