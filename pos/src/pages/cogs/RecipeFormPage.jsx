...
const RecipeFormPage = () => {
  ...
  const fetchRecipeData = useCallback(async (recipeIdToFetch) => {
    ...
    setFormData({
      ...
      produces_item_id: recipeData.produces_item?.id?.toString() || "", // from produces_item (object) to id string
      yield_quantity: recipeData.yield_quantity?.toString() || "1.0",
      // Assuming recipeData.yield_unit is an object with an id, or just an ID
      yield_unit_id:
        recipeData.yield_unit?.id?.toString() ||
        recipeData.yield_unit?.toString() ||
        "",
    });

    if (recipeData.components && recipeData.components.length > 0) {
      const populatedComponents = recipeData.components.map((comp) => {
        // comp.inventory_item is likely an object with id, comp.quantity_unit is also likely object with id
        return {
          inventory_item_id: comp.inventory_item?.id?.toString() || "",
          quantity: comp.quantity?.toString() || "1",
          unit_id: comp.quantity_unit?.id?.toString() || "", // Map from backend's quantity_unit.id to frontend's unit_id
        };
      });
  ...
  const fetchPageData = useCallback(async () => {
    ...
    setPageError(null);
    try {
      const { unitsData } = await fetchDropdownData();
      if (isEditMode && recipeId) {
        await fetchRecipeData(recipeId, unitsData);
      }
    } catch (err) {
    ...
    return (
      <MainLayout>
        <div className="max-w-2xl mx-auto space-y-6 p-4 text-center">
          ...
          <Button onClick={() => navigate("/cogs/recipes")} variant="outline">
            Back to Recipes
          </Button>
        </div>
      </MainLayout>
